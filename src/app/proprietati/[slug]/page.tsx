          ...
          <div className="mt-10 pt-8 border-t flex flex-col items-center justify-between gap-6">
             ...
             {(p.status === 'activ' || p.status === 'Activă') && (
               <div className="w-full flex flex-col md:flex-row gap-4 justify-center">
                 <a href={`tel:${p.agent_phone || settings.contact_phone || '0700000000'}`} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-4 rounded-xl font-bold text-lg transition-transform transform hover:scale-105 shadow-md flex-1">
                   <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                   Sună Acum
                 </a>
                 <a href={`https://wa.me/${(p.agent_whatsapp || settings.contact_whatsapp || '0700000000').replace(/[^0-9]/g, '').replace(/^0/, '40')}?text=${encodeURIComponent(`Bună ziua, vă contactez în legătură cu anunțul: ${p.title}`)}`} target="_blank" className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-6 py-4 rounded-xl font-bold text-lg transition-transform transform hover:scale-105 shadow-md flex-1">
                 ...
                 <ShareButton 
                   url={`/proprietati/${p.slug}`}
                   title={`Siena Imobiliare: ${p.title}`}
                   text={p.description?.substring(0, 100) + '...'}
                   showText={true}
                   className="flex items-center justify-center gap-2 bg-gray-800 hover:bg-gray-900 text-white px-6 py-4 rounded-xl font-bold text-lg transition-transform transform hover:scale-105 shadow-md flex-1"
                 />
               </div>
             )}
          </div>
